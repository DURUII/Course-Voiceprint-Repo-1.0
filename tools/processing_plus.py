...
def addnoise(speech, noise_list, snr_dur=[0, 20], max_frames=200):
    ...
    noise = truncate_speech(load_wav(random.choice(
        noise_list[noies_type])), max_frames=max_frames, train_mode=True)
...
def _get_sample(path, resample=None):
    ...
    return torchaudio.sox_effects.apply_effects_tensor(torchaudio.load(path)[0], torchaudio.load(path)[1], effects=effects)
...
def load_wav(path, max_frames=200, fs=16000, train_mode=False):
    ...
    if train_mode:
        ...
        if signalsize <= max_audio:
            ...
            startframe = np.array(
                [np.int64(random.random()*(signalsize-max_audio))])[0]
...
def augment_spec(feat, opt):
    ...
    if random.sample(['time', 'freq'], 1)[0] == 'time':
        param = random.randint(
            opt.specaug_masktime[0], opt.specaug_masktime[1])
        masking = T.TimeMasking(time_mask_param=param)
    else:
        param = random.randint(
            opt.specaug_maskfreq[0], opt.specaug_maskfreq[1])
